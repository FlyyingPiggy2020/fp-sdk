...
static const uint32_t export_id_table[EXPORT_MAX + 1] = {
    EXPORT_ID_INIT, EXPORT_ID_INIT, EXPORT_ID_INIT, EXPORT_ID_INIT, EXPORT_ID_INIT, EXPORT_ID_INIT, EXPORT_ID_POLL,
};
    ...
    for (uint32_t i = 0;; i++) {
        if (export_table[i].magic_head == export_id && export_table[i].magic_tail == export_id) {
            if (export_table[i].level == level && level <= EXPORT_APP) {
                ...
                if (((_time >= data->timeout_ms) && (_time - data->timeout_ms < ELAB_POLL_PERIOD_MAX))
                    || ((_time < data->timeout_ms) && (data->timeout_ms - _time > ELAB_POLL_PERIOD_MAX))) {
