 ...
 * @LastEditTime : 2025-09-29 14:18:32
 * @Brief        :
 */
...
void serial_transport_poll(struct serial_bus_handle *self)
{
    do {
        ...
        do {
            if (n->trans_cnt < n->trans_max_cnt) {
                n->trans_cnt++;
                self->ops->write_buf(n->payload, n->data_len);
                if (n->trans_cnt == n->trans_max_cnt && n->trans_max_cnt != SERIAL_BUS_TRANS_FOREVER) {
                    list_del(&n->node);
                    __free(n);
                    n = NULL;
                }
...
void serial_bus_send_to_cache(
    struct serial_bus_handle *self, const uint8_t *pdata, uint32_t length, uint8_t priority, uint8_t trans_cnt)
{
    do {
        if (self == NULL) {
            break;
        ...
        struct serial_bus_transport_node *node =
            (struct serial_bus_transport_node *)malloc(sizeof(struct serial_bus_transport_node) + length);
        if (node == NULL) {
            break;
        }
...
void serial_bus_receive(struct serial_bus_handle *self, const uint8_t *pdata, uint32_t length)
{
    if (self->cb && self->cb->recv_phare_callback) {
        self->cb->recv_phare_callback(
            (uint8_t *)pdata, length, self->cur_tx_node->payload, self->cur_tx_node->data_len);
    }
}

void serial_bus_clear_txlist(struct serial_bus_handle *self)
{
    do {
        if (self == NULL) {
            break;
