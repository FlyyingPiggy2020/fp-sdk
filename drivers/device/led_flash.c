...
static int32_t _ledf_open(driver_t **pdrv)
{
    ...
    do {
        ...
        pdesc->priv.led_flash_list = __malloc(sizeof(led_flash_logic_t) * pdesc->led_num);
 ...
 */
int32_t
__led_start_flash(ledf_describe_t *pdesc, uint8_t id, uint16_t cnt, uint16_t delay, led_flash_layer_name_t layer)
{
    if (!pdesc->priv.led_flash_list) {
        return E_POINT_NONE;
    }
