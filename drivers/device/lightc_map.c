 ...
 * @LastEditTime : 2025-09-11 10:44:33
 * @Brief        :
 */
...
static int32_t __light_reverse_brightness_to_remeber(lightc_map_describe_t *pdesc, void *args);
...
static int32_t __light_reverse_color_to_remeber(lightc_map_describe_t *pdesc, void *args);
...
static int32_t __light_reverse_color(lightc_map_describe_t *pdesc, void *args);
static int32_t __light_tips_start(lightc_map_describe_t *pdesc, void *args);
static int32_t __light_tips_stop(lightc_map_describe_t *pdesc, void *args);
/*---------- variable ----------*/
...
static struct protocol_callback ioctl_cbs[] = {
    ...
    { IOCTL_LIGHTC_REVERSE, __light_reverse_brightness_to_remeber },
    ...
    { IOCTL_LIGHTC_TIPS_START, __light_tips_start },
    { IOCTL_LIGHTC_TIPS_STOP, __light_tips_stop },
};
/*---------- function ----------*/
...
static int32_t _light_open(driver_t **pdrv)
{
    ...
    do {
        ...
        if (pdesc->param.start_state == 0) {
            ...
            __light_param_read(pdesc, NULL);
            param.set.brightness = pdesc->param.poweron_brightness;
            __light_set_brightness(pdesc, &param);
        }
    } while (0);
    ...
    if (pdesc) {
        ...
        do {
            if (pdesc->priv.mode == LIGHTC_MAP_MODE_NORMAL || pdesc->priv.mode == LIGHTC_MAP_MODE_LOOP) {
                ...
                if (pdesc->priv.tips.is_tips) {
                    pdesc->priv.tips.is_tips = false;
                }
...
static void __init_priv_param(lightc_map_describe_t *pdesc)
{
    ...
    if (pdesc->param.start_delay) {
        pdesc->priv.brightness_step_1_percent_inc =
            1 * 10 / (LIGHT_MAP_FLOAT_POINT_TYPE)(pdesc->param.start_delay * pdesc->time_slice_frequence);
    } else {
        pdesc->priv.brightness_step_1_percent_inc = 1;
    }
    if (pdesc->param.start_delay) {
        pdesc->priv.brightness_step_1_percent_dec =
            1 * 10 / (LIGHT_MAP_FLOAT_POINT_TYPE)(pdesc->param.stop_delay * pdesc->time_slice_frequence);
    } else {
    ...
    pdesc->priv.brightness_step_1_to_100_inc =
        99 / (LIGHT_MAP_FLOAT_POINT_TYPE)(pdesc->time_slice_frequence * pdesc->param.fade_in_time);
    pdesc->priv.brightness_step_1_to_100_dec =
        99 / (LIGHT_MAP_FLOAT_POINT_TYPE)(pdesc->time_slice_frequence * pdesc->param.fade_out_time);
    pdesc->priv.color_step_inc =
        (pdesc->priv.color_max - pdesc->priv.color_min)
        / (LIGHT_MAP_FLOAT_POINT_TYPE)(pdesc->time_slice_frequence * pdesc->param.fade_cold_time);
    pdesc->priv.color_step_dec =
        (pdesc->priv.color_max - pdesc->priv.color_min)
        / (LIGHT_MAP_FLOAT_POINT_TYPE)(pdesc->time_slice_frequence * pdesc->param.fade_warm_time);

    if (pdesc->param.poweron_brightness > 100) {
        pdesc->param.poweron_brightness = 0;
    }
}
...
static void _get_brightness_actual_from_brightness(lightc_map_describe_t *pdesc)
{
    ...
    if (pdesc->param.dimming_start_point == 0) {
        start_point =
            (LIGHT_MAP_FLOAT_POINT_TYPE)(pdesc->param.dimming_end_point - pdesc->param.dimming_start_point) / 100;
...
static void _get_color_pwm(lightc_map_describe_t *pdesc)
{
    ...
    x1 = pdesc->priv.color_min;
    y1 = 0;
    x2 = pdesc->priv.color_max;
    y2 = 1;
...
static int32_t __light_reverse_ext(lightc_map_describe_t *pdesc, void *args)
{
    ...
    do {
        if (pdesc->is_virtual == true) {
            if (pdesc->xfer.lightc_cmd_reverse_ext) {
                pdesc->xfer.lightc_cmd_reverse_ext(param->set.resever_cmd);
            }
        } else {
            if (param->set.resever_cmd == 1) {
                // 1：100%单键开关灯循环
                __light_reverse_brightness(pdesc, NULL);
            } else if (param->set.resever_cmd == 3) {
                // 3：最暖最冷色温循环
                __light_reverse_color(pdesc, NULL);
            } else if (param->set.resever_cmd == 4) {
                // 4: 亮度调光反向
                __light_reverse_brightness_to_remeber(pdesc, NULL);
            } else if (param->set.resever_cmd == 5) {
                // 5: 色温调光反向
                __light_reverse_color_to_remeber(pdesc, NULL);
            }
        }
    } while (0);
    return err;
...
{
    int32_t err = DRV_ERR_EOK;
    if (pdesc->priv.color_status == LIGHTC_MAP_STATUS_STOP && pdesc->color != pdesc->priv.color_max) {
        union lightc_map_param param;
        param.set.color = pdesc->priv.color_max;
        __light_set_color(pdesc, &param);
    } else if (pdesc->priv.color_status == LIGHTC_MAP_STATUS_STOP && pdesc->color == pdesc->priv.color_max) {
        union lightc_map_param param;
        param.set.color = pdesc->priv.color_min;
        __light_set_color(pdesc, &param);
    } else {
        if (pdesc->priv.color_status == LIGHTC_MAP_STATUS_INC) {
            union lightc_map_param param;
...
static int32_t __light_cmd_warm(lightc_map_describe_t *pdesc, void *args)
{
    int32_t err = DRV_ERR_WRONG_ARGS;
    ...
    do {
        ...
        if (pdesc->is_virtual == false) {
            ...
            pdesc->priv.color_postion = pdesc->priv.color_min;
            err = DRV_ERR_EOK;
            break;
        }
...
static int32_t __light_cmd_cold(lightc_map_describe_t *pdesc, void *args)
{
    int32_t err = DRV_ERR_WRONG_ARGS;
    double brightness = pdesc->priv.remeber_color;
    ...
    do {
        ...
        if (pdesc->is_virtual == false) {
            pdesc->priv.color_mode = LIGHTC_MAP_MODE_NORMAL;
            pdesc->priv.color_postion = brightness;
            err = DRV_ERR_EOK;
            break;
        }
    } while (0);
...
static int32_t __light_reverse_color_to_remeber(lightc_map_describe_t *pdesc, void *args)
{
    ...
    do {
        if (pdesc->is_virtual == true) {
            ...
            if (pdesc->priv.color_status == LIGHTC_MAP_STATUS_STOP && pdesc->color != pdesc->priv.color_min) {
                __light_cmd_warm(pdesc, args);
            } else if (pdesc->priv.color_status == LIGHTC_MAP_STATUS_STOP && pdesc->color == pdesc->priv.color_min) {
                __light_cmd_cold(pdesc, args);
            } else {
                if (pdesc->priv.color_status == LIGHTC_MAP_STATUS_INC) {
                    __light_cmd_warm(pdesc, args);
                } else if (pdesc->priv.color_status == LIGHTC_MAP_STATUS_DEC) {
...
static int32_t __light_param_write(lightc_map_describe_t *pdesc, void *args)
{
    ...
    pdesc->param.fade_cold_time = param->param.fade_cold_time;
    pdesc->param.fade_warm_time = param->param.fade_warm_time;
    // 重新初始化计算参数
    __init_priv_param(pdesc);
    return err;
}
...
static int32_t __light_set_color(lightc_map_describe_t *pdesc, void *args)
{
    ...
    do {
        ...
        if (param->set.color > pdesc->priv.color_max || param->set.color < pdesc->priv.color_min) {
            break;
        }

        if (pdesc->is_virtual == true) {
            if (pdesc->xfer.lightc_cmd_set_color) {
                pdesc->xfer.lightc_cmd_set_color(param->set.color);
            }
            err = DRV_ERR_EOK;
            break;
        }
...
static int32_t __light_tips_start(lightc_map_describe_t *pdesc, void *args)
{
    ...
    do {
        ...
        pdesc->priv.tips.is_tips = true;
        pdesc->priv.tips.last_brightness = pdesc->brightness;
        pdesc->priv.mode = LIGHTC_MAP_MODE_LOOP_BY_TIME;
...
static int32_t __light_tips_stop(lightc_map_describe_t *pdesc, void *args)
{
    ...
    double brightness = pdesc->priv.tips.last_brightness;
