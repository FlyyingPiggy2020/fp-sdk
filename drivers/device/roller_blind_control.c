...
static void _motor_speed_control(roller_blind_control_describe_t *pdesc)
{
    ...
    do {
        ...
        // 知道起始点和终止点，速度利用纯比例调节，误差为当前位置和行程中间点的距离，这样可以实现一个慢启慢停的效果。
        if (pdesc->config->route_up != MOTOR_ROUTE_FREE && pdesc->config->route_down != MOTOR_ROUTE_FREE) {
            // slow down and slow up when close to target
            int32_t route_e =
                pdesc->priv.speed.half_route - abs32(pdesc->priv.speed.half_point, pdesc->config->route_curr);
...
static void _motor_resistance_control(roller_blind_control_describe_t *pdesc)
{
    ...
    if (pdesc->ops.get_motor_current) {
        if (pdesc->priv.current.mutex == 0) {
            if (pdesc->priv.current.load > 600) {
...
static bool __motor_goto_route(roller_blind_control_describe_t *pdesc, int32_t route)
{
    ...
    do {
        ...
        pdesc->priv.speed.half_route = abs32(pdesc->priv.control.target_route, pdesc->config->route_curr) / 2;
        pdesc->priv.speed.half_point = (pdesc->priv.control.target_route + pdesc->config->route_curr) / 2;
